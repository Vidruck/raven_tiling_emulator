            ...
            "swap_next" | "swap_prev" => {
                ...
                if active_windows.len() > 1 {
                    ...
                    if let Some(ref active_id) = active_window_id {
                        if let Some(current_idx) = active_windows.iter().position(|w| &w.window_id == active_id) {
                            ...
                            let pos_active = self.engine.window_history.iter().position(|id| id == active_id);
                            let pos_target = self.engine.window_history.iter().position(|id| id == target_id);
