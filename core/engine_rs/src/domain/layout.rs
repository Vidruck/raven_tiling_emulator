    ...
    for j in 0..num_windows {
        let h = if num_windows == 1 {
            ...
            if j == active_idx {
                ...
                (container_height as f32 * (1.0 - safe_split_ratio) / (num_windows - 1) as f32) as i32
            }
        } else {
...
pub fn calculate_master_stack(
    ...
    loop {
        ...
        let active_idx = if let Some(ref active_id) = active_window_id {
            ordered_active.iter().position(|w| w.window_id == *active_id)
        } else {
            None
        };
        ...
        if count <= nmaster_temp {
            ...
            let master_active_idx = if let Some(idx) = active_idx {
                if idx < nmaster_temp { Some(idx) } else { None }
            } else {
                None
            };

            let stack_active_idx = if let Some(idx) = active_idx {
                if idx >= nmaster_temp { Some(idx - nmaster_temp) } else { None }
            } else {
                None
            };
