...
fn create_test_window(id: &str, workspace_id: &str, output: &str, is_floating: bool) -> WindowNode {
    WindowNode {
...
async fn test_fullscreen_direct_transition_evicts_from_tiling_background() {
    ...
    let actions = controller
        .handle_state_change(
            workspaces.clone(),
            vec![nav_window.clone(), term_window.clone()],
        )
    ...
    if let Some(raven_core::action::RavenAction::MoveWindow {
        window_id,
        width,
        height,
        ..
    }) = fs_actions.first()
    {
        ...
        assert!(
            ...
            "Terminal debe ocupar el espacio vacante"
        );
    }
    ...
    for action in fs_actions {
        ...
        } = action
        {
            ...
            if window_id == "win-m2-2" {
                assert!(
                    (1920..=1936).contains(&x),
                    "Monitor 2 debe ubicarse con respecto a x=1920"
                );
